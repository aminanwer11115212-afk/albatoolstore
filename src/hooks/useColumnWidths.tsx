...
export function useColumnWidths(
  ...
  const minFor = useCallback(
    (index: number) => {
      ...
      // أرضيةٌ واحدة صغيرة لكل الأعمدة بدل `min(d, 60)`.
      //
      // الأرضية القديمة كانت تمنع تضييق أيّ عمود تحت 60px، فعمودٌ كـ«السعر
      // الأجنبي» لا يُضيَّق ولو لم يُستعمل — والمستخدم يعرف شاشته وحاجته أكثر
      // من رقمٍ ثابت في الكود. 24px تكفي ليبقى مقبضُ السحب ملموساً.
...
export function useContainerFit(
  ...
  useEffect(() => {
    ...
    const run = () => {
      ...
      /**
       * الملاءمة التلقائية تُضيّق الأعمدة لتسع الحاوية. وكانت تعمل **أثناء
       * التعديل** وتتوقّف بعد الحفظ — عكس ما ينبغي تماماً: يسحب المستخدم عموداً
       * فتُعيده إلى مكانه، فيظنّ العرض مقيّداً وهو مُقاوَم.
       *
       * صارت جافّة في وضع التعديل: ما يسحبه المستخدم يثبت، والجدول ينزلق
       * أفقياً إن تجاوز الحاوية. وتعمل حقيقةً بعد القفل، حيث لا يد تنازعها.
       */
