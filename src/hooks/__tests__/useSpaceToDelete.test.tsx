...
describe("useSpaceToDelete — وضع التنقّل ↔ وضع التحرير عبر Enter", () => {
  ...
  it("النقر خارج الحقل (mousedown خارج + blur) يُعيده لوضع التنقّل حتى بعد Enter → Space لا يُحدِّد ولا يحذف", () => {
    const onDelete = vi.fn();
    const { result } = renderHook(() => useSpaceToDelete(onDelete));

    const input = mountTextInput("مسمار");
    // ادخل وضع التحرير عبر Enter
    fireKey(input, "Enter");

    // نقر بالماوس على عنصر خارج الحقل (زر مثلاً)
    ...
    // حتى لو ضغط Enter ثانية داخل الحقل ثم نقر خارجه — يظل Space لا يُحدِّد
    const ev = makeReactKeyEvent(input);
    act(() => result.current.handleRowKeyDown("uid-out-1", ev));
  ...
  it("Enter ثم نقر خارج ثم Enter من جديد يجب أن يُعيد وضع التحرير فقط بعد Enter الجديد", () => {
    const onDelete = vi.fn();
    const { result } = renderHook(() => useSpaceToDelete(onDelete));

    const input = mountTextInput("لوح");
    fireKey(input, "Enter"); // تحرير
    ...
    // بعد الرجوع، النص كامل محدَّد (Tab-like) → وضع تنقّل، Space لا يفعل شيئاً
    input.setSelectionRange(0, input.value.length);
    const ev1 = makeReactKeyEvent(input);
    act(() => result.current.handleRowKeyDown("uid-out-2", ev1));
    expect(result.current.pendingUids.size).toBe(0);
    expect(onDelete).not.toHaveBeenCalled();

    // اضغط Enter من جديد → وضع تحرير، Space يكتب مسافة (لا تحديد)
    fireKey(input, "Enter");
    const ev2 = makeReactKeyEvent(input);
    act(() => result.current.handleRowKeyDown("uid-out-2", ev2));
    expect((ev2 as any).defaultPrevented).toBe(false);
    expect(result.current.pendingUids.size).toBe(0);
  });
  });

  it("حقل رقمي مملوء مسبقاً → Space يُحدِّد الصف كاختصار (لا يتأثر بالقيمة)", () => {
    const onDelete = vi.fn();
    const { result } = renderHook(() => useSpaceToDelete(onDelete));
    ...
    const ev = makeReactKeyEvent(input);
    act(() => result.current.handleRowKeyDown("uid-n1", ev));

    expect((ev as any).defaultPrevented).toBe(true);
    expect(result.current.pendingUids.has("uid-n1")).toBe(true);
  });

  it("حقل رقمي مملوء + Enter سابق → Space يستمر كاختصار تحديد (Enter لا يفعّل وضع تحرير على الأرقام)", () => {
    const onDelete = vi.fn();
    const { result } = renderHook(() => useSpaceToDelete(onDelete));
    ...
    const ev = makeReactKeyEvent(input);
    act(() => result.current.handleRowKeyDown("uid-n2", ev));

    expect((ev as any).defaultPrevented).toBe(true);
    expect(result.current.pendingUids.has("uid-n2")).toBe(true);
  });

  it("حقل رقمي مملوء + نقر ماوس داخله → Space يظل يعمل كاختصار حذف بضغطتين", async () => {
    const onDelete = vi.fn();
    const { result } = renderHook(() => useSpaceToDelete(onDelete));
    ...
    act(() => result.current.handleRowKeyDown("uid-n3", makeReactKeyEvent(input)));
    act(() => result.current.handleRowKeyDown("uid-n3", makeReactKeyEvent(input)));

    await new Promise((r) => setTimeout(r, 0));
    expect(onDelete).toHaveBeenCalledWith("uid-n3");
  });

  it("SELECT بقيمة محدَّدة → Space يُحدِّد الصف (اختصار)", () => {
    const onDelete = vi.fn();
    const { result } = renderHook(() => useSpaceToDelete(onDelete));
