...
/**
 * شارة رصيد بإشارة ولون: «له +X» أخضر، «عليه −X» أحمر، «خالص» محايد.
 * توكنات التصميم فقط — لا ألوان مثبّتة.
 */
function Chip({ text, tone }: { text: string; tone: "debit" | "credit" | "settled" }) {
  const cls =
    tone === "credit"
  ...
  return (
    <span
      data-tone={tone}
      className={`inline-block whitespace-nowrap rounded-full border px-2 py-0.5 text-xs font-bold tabular-nums ${cls}`}
    >
      {text}
    </span>
  );
...
/** رصيد بلغة «له/عليه» — للبطاقات العلوية والرصيد الصافي. */
const BalanceChip = ({ value }: { value: number }) => <Chip {...signedBalanceText(value)} />;
/** رقم موقّع مختصر لعمود «المتبقي»: «+300» أخضر / «−100» أحمر. */
const AmountChip = ({ value }: { value: number }) => <Chip {...signedAmountText(value)} />;
...
export default function CustomerStatementPage() {
  ...
  const invoiceNumberById = useMemo(() => {
    const m = new Map<string, string>();
    for (const inv of (invoices as any[]) || []) m.set(inv.id, inv.invoice_number || "");
    return m;
  }, [invoices]);
  const openDeleteEntry = (t: any) => {
    setDeleteEntry({
      ...t,
      invoiceNumber: t.reference_id ? invoiceNumberById.get(t.reference_id) || null : null,
  ...
  // ===== حساب العميل مجمّعاً حسب الفاتورة =====
  // كل فاتورة تتبعها حركاتها الحقيقية فقط (المرتبطة بها صراحةً)، والرصيد
  // الدائن غير المرتبط يبقى قابلاً للتوزيع يدوياً — بلا توزيع تلقائي.
  // يُبنى بفلترة التاريخ فقط حتى لا تُشوّه فلاتر الجداول الأخرى الرصيد الجاري.
  ...
  // كتل الفواتير المعروضة: نفس فلاتر جدول الفواتير مطبَّقة على العرض المجمّع.
  ...
  // أسطر الجدول المعروضة: ما يخصّ فاتورة يتبع فلتر الفواتير، وأسطر الرصيد
  // غير المرتبطة تُخفى عند البحث برقم فاتورة لأنها لا تخصّ فاتورة بعينها.
  const visibleRows = useMemo(() => {
    const allowed = new Set(visibleBlocks.map((b) => b.invoiceId));
    const searching = invSearch.trim().length > 0;
    return accountView.rows.filter((r) =>
      ...
      sections: [
        {
          ...
          label: "الحساب",
          columns: [
            { key: "seq", label: "#", align: "center" as const },
          ...
          totals: {
            seq: "",
            when: "المجموع",
            total: accountView.rowsValue,
            paid: accountView.rowsPaid,
            remaining: signedAmountText(-accountView.accountTotal).text,
          },
        },
        ...
        <>
          ...
          <div data-section="invoices" data-section-label="الفواتير" className={`legacy-card card-block ${tab === "invoices" ? "" : "hidden"}`}>
            <div className="px-5 py-3 border-b border-border flex flex-wrap items-center gap-2 justify-between">
              <div>
                <h3 className="font-semibold text-foreground">الفواتير وحركاتها ({visibleBlocks.length})</h3>
                <p className="text-xs text-muted-foreground mt-0.5">
                  كل فاتورة تتبعها حركاتها المرتبطة بها فقط بدقة الساعة — لا توزيع تلقائي للرصيد.
                </p>
              </div>
              <div className="relative">
                ...
                <tbody>
                  ...
                  {!isLoading && visibleRows.length > 0 && (
                    <tr className="bg-muted font-bold border-t-2 border-border">
                      ...
                      <td className="px-5 py-3 text-foreground">المجموع</td>
                      <td className="px-5 py-3 text-muted-foreground text-xs">
                        {accountView.blocks.length} فاتورة
                      </td>
                      <td className="px-5 py-3 text-foreground tabular-nums">{accountView.rowsValue.toLocaleString()}</td>
                      <td className="px-5 py-3 text-success tabular-nums">{accountView.rowsPaid.toLocaleString()}</td>
                      <td className="px-5 py-3"><AmountChip value={-accountView.accountTotal} /></td>
                    </tr>
                  )}
                </tbody>
              </table>
                ...
                <table className="w-full text-sm mobile-stack-table">
                  <thead>
                    <tr className="bg-muted">
                      ...
                      <th className="text-right px-5 py-3 font-semibold text-muted-foreground print:hidden">إجراءات</th>
                    </tr>
                  </thead>
                  <tbody>
