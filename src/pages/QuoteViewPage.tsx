...
export default function QuoteViewPage() {
  ...
  const loadQuote = async () => {
    ...
    setLoading(true);
    try {
      const { data: q, error: qErr } = await supabase.from("quotes").select("*, customers(name, phone, whatsapp, email, address, balance, credit_balance, net_balance)").eq("id", id).single();
      if (qErr) {
