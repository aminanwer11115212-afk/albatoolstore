...
interface Props {
  ...
  shippedTotals?: Record<string, number>;
  onItemsChanged?: () => void;
}

export default function TransportItemsManager({
  ...
  const products = docProducts.filter((p: any) => {
    ...
    const shipped = shippedTotals[p.id] ?? 0;
    if (total === 0) return true;
    return total - shipped > 0;
  });
  ...
  const add = async () => {
    ...
    if (productId) {
      ...
      const shipped = shippedTotals[productId] ?? 0;
      const available = total - shipped;
  ...
  return (
    <div style={{ padding: "0.75rem 1rem", borderTop: "1px solid hsl(var(--border))" }}>
      <h5 style={{ fontSize: "0.85rem", fontWeight: 600, marginBottom: "0.5rem", color: "hsl(var(--muted-foreground))" }}>
        بنود الترحيل ({items.length}) {totalSum > 0 && <span> — إجمالي: {totalSum.toLocaleString()}</span>}
      </h5>

      <div style={{ display: "grid", gridTemplateColumns: "1.6fr 90px 90px 90px 90px auto", gap: "0.4rem", alignItems: "end", marginBottom: "0.5rem" }}>
        <div>
          <label style={{ fontSize: 11, color: "hsl(var(--muted-foreground))", display: "block", marginBottom: 2 }}>المنتج</label>
          <Popover open={productPickerOpen} onOpenChange={setProductPickerOpen}>
            <PopoverTrigger asChild>
              <button type="button" role="combobox" aria-expanded={productPickerOpen}
            ...
            <PopoverContent className="p-0 w-[320px]" align="start" dir="rtl">
              <Command filter={(value, search) => !search ? 1 : value.toLowerCase().includes(search.toLowerCase()) ? 1 : 0}>
                ...
                <CommandList>
                  ...
                  <CommandGroup>
                    <CommandItem value="" onSelect={() => { setProductId(""); setProductName(""); setProductPickerOpen(false); }}>
        ...
        <div>
          ...
          <input type="number" className="legacy-control" value={packsCount} onChange={(e) => setPacksCount(e.target.value)} />
        </div>
        <div>
          <label style={{ fontSize: 11, color: "hsl(var(--muted-foreground))", display: "block", marginBottom: 2 }}>قطع/عبوة</label>
          <input type="number" className="legacy-control" value={piecesPerPack} onChange={(e) => setPiecesPerPack(e.target.value)} />
        </div>
        <div>
        ...
        <div>
          ...
          <input type="number" className="legacy-control" value={price} step="0.01"
      ...
      {items.length > 0 && (
        <table className="legacy-table" style={{ fontSize: 12 }}>
          <thead>
            <tr>
              <th>المنتج</th>
              <th className="text-center" title="مُرحَّل من إجمالي الفاتورة">مُرحَّل / إجمالي</th>
          ...
          <tbody>
            {(items as any[]).map((it, idx) => {
              ...
              const total = qty * pr;
              return (
                <tr key={it.id} className={idx % 2 === 0 ? "odd" : "even"}>
                  <td>{it.product_name || "—"}</td>
                  <td className="text-center" style={{ fontWeight: 600, color: "hsl(var(--muted-foreground))" }}>
                    {it.product_id
                      ? `${shippedTotals[it.product_id] ?? 0} / ${productQuantities[it.product_id] ?? 0}`
                      : "—"}
                  </td>
                  <td className="text-center">
                    <div style={{ display: "inline-flex", alignItems: "center", gap: 4 }}>
                      <input type="number" className="legacy-control" value={d.packs}
                        onChange={(e) => updateDraft(it.id, "packs", e.target.value)}
                        onBlur={() => flushSave(it.id)}
                        style={{ width: 60, textAlign: "center" }} />
                      <span>×</span>
                      <input type="number" className="legacy-control" value={d.pieces}
                  ...
                  <td className="text-center">
                    <input type="number" className="legacy-control" value={d.price} step="0.01"
