...
export default function QuoteAttachmentsDialog({ quoteId, open, onClose }: Props) {
  ...
  const load = async () => {
    ...
    if (error) toast.error(error.message);
      ...
      const signed = await resolveAttachmentSignedUrls(list, "quote-attachments");
      setItems(signed);
    }
    setLoading(false);
  };
  ...
  useEffect(() => {
    if (open && quoteId) load();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [open, quoteId]);

  const handleUpload = async (files: FileList | null) => {
    if (!files || !quoteId || activeTab === "trash") return;
    setUploading(true);
    try {
      for (const file of Array.from(files)) {
        const ext = file.name.split(".").pop();
        const path = `${quoteId}/${activeTab}/${Date.now()}-${Math.random().toString(36).slice(2, 8)}.${ext}`;
        const { error: upErr } = await supabase.storage
          .from("quote-attachments")
          .upload(path, file, { contentType: file.type });
        if (upErr) throw upErr;
        const { data: pub } = supabase.storage.from("quote-attachments").getPublicUrl(path);
        const { error: insErr } = await supabase.from("quote_attachments").insert({
          quote_id: quoteId,
      ...
      <div className="bg-card rounded-xl border border-border shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col">
        <div className="flex items-center justify-between px-4 py-3 border-b border-border">
          <h3 className="text-base font-bold text-foreground flex items-center gap-2">
            <Paperclip size={18} /> مستندات عرض السعر
          </h3>
          ...
          <div className="p-3 border-b border-border">
            {!quoteId ? (
              <div className="text-sm text-muted-foreground bg-muted/40 rounded-lg p-3 text-center">
                يرجى حفظ عرض السعر أولاً قبل إرفاق المستندات.
              </div>
            ) : (
