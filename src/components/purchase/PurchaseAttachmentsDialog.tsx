...
export default function PurchaseAttachmentsDialog({ purchaseOrderId, open, onClose }: Props) {
  ...
  const hardDelete = async (att: Attachment) => {
    ...
    try {
      const marker = "/purchase-attachments/";
      ...
      <div className="bg-card rounded-xl border border-border shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col">
        <div className="flex items-center justify-between px-4 py-3 border-b border-border">
          <h3 className="text-base font-bold text-foreground flex items-center gap-2">
            <Paperclip size={18} /> مستندات أمر الشراء
          </h3>
          ...
          <div className="p-3 border-b border-border">
            {!purchaseOrderId ? (
              <div className="text-sm text-muted-foreground bg-muted/40 rounded-lg p-3 text-center">
                يرجى حفظ أمر الشراء أولاً قبل إرفاق المستندات.
              </div>
            ) : (
              ...
              {filtered.map((att) => {
                ...
                return (
                  ...
                  >
                    ...
                    {inTrash ? (
                      <>
                        <button onClick={() => restore(att)} className="p-1.5 rounded hover:bg-primary/10 text-primary" title="استرجاع">
                          <RotateCcw size={15} />
                        </button>
                        <button onClick={() => hardDelete(att)} className="p-1.5 rounded hover:bg-destructive/10 text-destructive" title="حذف نهائي">
                          <Trash2 size={15} />
                        </button>
                      </>
                    ) : (
                      <>
                        ...
                        <ShareFileButton url={att.file_url} fileName={att.file_name} mimeType={att.file_type} />
