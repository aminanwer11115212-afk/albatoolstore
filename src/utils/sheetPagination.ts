...
export const PAGINATION_CSS = `
  ...
  @media print {
    ...
    /*
     * === صفحةٌ فارغةٌ زائدة في آخر كل فاتورة ===
     *
     * الترقيمُ طبقةٌ مطلقة، فيحتاج ورقةً بطولِ صفحاتها كلِّها ليجد أين
     * يضع أرقامَه — ويكتبه السكربتُ سطرياً:
     *
     *     page.style.minHeight = (pages * pageH + padTop * 2) + 'px';
     *
     * والسطريُّ يغلب كلَّ قاعدةٍ في التنسيق إلا المُعلَّمةَ بـ!important.
     * فكان هذا الارتفاعُ يعبر إلى الطباعة: ورقةٌ محتواها صفحتان تُطبع
     * ثلاثاً، ثالثتُها بيضاء. قِيس في Chromium: محتوى 574mm على صفحةٍ
     * 277mm ⇒ ثلاث صفحات، امتلاءُ الأخيرة 7%.
     *
     * وهو ارتفاعٌ للمعاينة وحدها، فيُلغى هنا صراحةً — والطباعةُ تقسّم
     * بنفسها بلا حاجةٍ إليه. يحرسه «printSheetGeometry.test.ts».
     */
