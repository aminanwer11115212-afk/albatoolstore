...
/**
 * مقاسُ A4 بالمليمتر، وهامشُ الورقة — **المصدرُ الواحد لهندسة الورقة**.
 *
 * ## العطل الذي عالجه توحيدُ الهامش
 * كانت الورقةُ الواحدة تُقاس بهامشين:
 *
 *   • المعاينة على الشاشة: `.page { width: 210mm; padding: 10mm }`
 *   • الطباعة:             `@page { size: A4; margin: 10mm }`
 *   • الـPDF:              `A4_MM.margin = 8`
 *
 * فعرضُ المحتوى 190mm فيما يراه المستخدم، و**194mm** فيما ينزل ملفاً —
 * فرقٌ 4mm (نحو 2%). وهو لا يُرى رقماً بل يُرى **شكلاً**: الجدولُ يتمدّد،
 * والأعمدةُ تتغيّر، والأسطرُ تلتفّ في مواضعَ أخرى. فتخرج ورقةُ الـPDF غيرَ
 * الورقة التي عاينها صاحبُها — وهو ما تكرّر البلاغُ عنه.
 *
 * وينصّ CLAUDE.md صراحةً: «ومقاسُ الورقة على الشاشة — 210mm بهامش 10mm — في
 * القالب كذلك، فيراه العميل كما يراه الـPDF». فالعشرة هي الصواب، والثمانية
 * انحرافٌ عنها.
 *
 * فلا يُكتب هامشُ ورقةٍ رقماً في موضعٍ آخر: من احتاجه قرأه من هنا. يحرسه
 * `sheetGeometrySingleSource.test.ts`.
 */
export const A4_MM = { width: 210, height: 297, margin: 10 } as const;
