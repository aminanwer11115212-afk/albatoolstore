...
describe("مقاسُ A4", () => {
  /*
   * كان الهامشُ 8 هنا بينما الشاشةُ والطباعة على 10 — فيخرج المحتوى في
   * 194mm بدل 190mm، تمدُّدٌ نحو 2% يغيّر عرضَ الأعمدة ومواضعَ التفاف
   * الأسطر. فتُقرأ ورقةُ الـPDF غيرَ الورقة التي عاينها صاحبُها.
   *
   * وهذا الفحصُ كان يقيّد الانحرافَ نفسَه: يُثبّت الثمانية في مواجهة
   * `@page margin: 10mm` الذي يقيّده `printLayoutA4`. فاختباران متناقضان
   * لورقةٍ واحدة، وكلاهما أخضر.
   */
  it("210×297 بهامش 10 — نفس @page ونفس ورقة الشاشة", () => {
    expect(A4_MM).toEqual({ width: 210, height: 297, margin: 10 });
  });

  it("والمساحةُ الصالحة 190×277", () => {
