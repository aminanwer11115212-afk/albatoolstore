...
describe("share vs print: نفس بنية الورقة", () => {
  ...
  /**
   * الصندوقان موجودان في القالبين، وعنوانُ النقل وحده اختلف.
   *
   * فقد سُمّي في ورقتنا «تفاصيل الترحيل» ليطابق جارَه — تسميةُ صاحب المستودع
   * في طلبه. وقالبُ دالّة الحافة بقي على تسميته القديمة، ولا يُعدَّل: سياسةُ
   * لافابل ترفض تعديل `supabase/functions` من المستودع، فما يُكتب هناك لا
   * يصل الإنتاج مهما دُمج. وهو على أي حال مسارُ سقوطٍ للروابط القديمة وحدها
   * — رابطُ العميل اليوم يبني ورقتَه بالقالب نفسه.
   *
   * فالعقدُ هنا على **وجود** الصندوقين لا على حرف عنوانهما.
   */
  it("نفس صندوقَي التغليف والترحيل", () => {
    for (const html of [shareHtml, printHtml]) {
      expect(html).toContain("تفاصيل التغليف");
      expect(html).toMatch(/(تفاصيل|معلومات) الترحيل/);
    }
    // وورقتُنا وحدها هي التي تحمل التسمية الجديدة
    expect(printHtml).toContain("تفاصيل الترحيل");
  });
