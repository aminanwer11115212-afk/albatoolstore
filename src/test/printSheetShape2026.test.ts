...
describe("الألوانُ تُطبع كما تُعرض", () => {
  ...
  /**
   * والشريطُ صار فاتحَ الأرضية غامقَ الخطّ.
   *
   * «اجعل اللون خفيف كي لا يظهر في الطباعة أسود». الأرضيةُ الكحلية الغامقة
   * تخرج من الطابعة أحاديةِ اللون كتلةَ حبرٍ صلبة تبتلع الرقمَ الأبيضَ الذي
   * فيها — فيصير أبرزُ رقمٍ في الورقة أقلَّها قراءة. والتباينُ الآن أعلى لا
   * أدنى: غامقٌ على فاتحٍ يُقرأ مطبوعاً وعلى الشاشة سواء.
   */
  it("والشريطُ أرضيتُه فاتحة وخطُّه غامق — فلا كتلةَ حبرٍ في الطباعة", () => {
    const rule = /(?<![\w-.] )\.grand-band \{[^}]*\}/.exec(html)![0];
    expect(rule).toMatch(/background:\s*#eef2fb/);
    expect(rule).toMatch(/color:\s*#16224a/);
    // ولا عودةَ إلى الأرضية الداكنة ولا إلى الخطّ الأبيض عليها
    expect(rule).not.toMatch(/background:\s*#1f2d5a/);
    expect(rule).not.toMatch(/color:\s*#fff/);
    // ويبقى متميّزاً بإطاره لا بثقل حبره
    expect(rule).toMatch(/border:\s*[\d.]+px solid #1f2d5a/);
  });
});
...
describe("شريطُ الجملة شمالاً وفي مربّعين", () => {
  ...
  it("ومربّعان: للرقم أرضيةٌ أغمق قليلاً وفاصلٌ بينهما", () => {
    // «أغمق» ضمن الفاتح — يُقرأ الوصفُ خانةً والرقمُ خانةً كما في الصورة،
    // بلا أن تصير أيٌّ منهما كتلةً سوداء تحت الطابعة.
    const val = /(?<![\w-.] )\.grand-band-value \{[^}]*\}/.exec(html)![0];
    expect(val).toMatch(/background:\s*#dce4f5/);
