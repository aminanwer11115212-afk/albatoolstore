...
const EXPECTED = "فاتورة مبيعات - ايهاب الدين امدرمان - INV-61463 - 350,000.pdf";
...
describe("اسم الملف قبل أي مشاركة", () => {
  ...
  it("عرض السعر يأخذ نوعه لا نوع الفاتورة", () => {
    expect(pdfNameFor({ ...DOC, docLabel: "عرض سعر", docNumber: "QT-12" }))
      .toBe("عرض سعر - ايهاب الدين امدرمان - QT-12 - 350,000.pdf");
  });

  it("مستند بلا رقم أو مبلغ لا يترك فواصل فارغة", () => {
    expect(pdfNameFor({ ...DOC, docNumber: null, total: 0 }))
      .toBe("فاتورة مبيعات - ايهاب الدين امدرمان.pdf");
  });
});
